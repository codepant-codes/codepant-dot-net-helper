...
namespace CodePant.DotNet.Helper.Email
{
    public class EmailHelper : IEmailHelper
    {
        ...
        public void SendEmail(MailAddress fromAddress, List<MailAddress> toAddress, string subject, string body,
        ...
        public void SendEmail(string fromAddress, List<string> toAddress, string subject, string body,
            List<string> attachments)
        {
            List<MailAddress> maiLAddressList = new();

            for (int i = 0; i < toAddress.Count; i++)
            {
                maiLAddressList.Add(new MailAddress(toAddress[i]));
            }

            SendEmail(new MailAddress(fromAddress), maiLAddressList, subject, body, attachments);
        }
        ...
        {
            List<MailAddress> maiLAddressList = new();
            maiLAddressList.Add(new MailAddress(toAddress));
